...
type Dbscript struct{
	...
	DbVersion string `json:"dbversion" form:"dbversion"`
...
func(p *Dbscript)UploadDbcheck()error{
	...
	res,err := sql.Pool.Exec("insert into ops_dbscript (`scriptname`,`version`,`app`,`appversion`,`branch`,`filepath`,`createtime`,`comment`) values (?,?,?,?,?,?,?,?) on duplicate key update filepath=?",p.Name,p.DbVersion,p.App,p.AppVersion,p.Branch,p.FilePath,p.CreateTime,p.Comment,p.FilePath)
	if err != nil {
		return err 
	}
	fmt.Print(res)
